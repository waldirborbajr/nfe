...
func main() {
	...
	mux.HandleFunc("/login", handler.LoginHandler(db.(*repository.DBConnSQLite), config))
	mux.HandleFunc("/login/submit", handler.LoginSubmitHandler(db.(*repository.DBConnSQLite), config))
